...
LAME_LAMDA = 1.25  
DENSITY = 1.0
ACCELERATION_DUE_TOGRAVITY = 0.016
...
def solve_cantilever_beam():
    ...
    def sigma(u):
        cauchy_stress = (
            LAME_LAMDA * fe.tr(epsilon(u)) * fe.Identity(3)
            +
            2 * LAME_MU * epsilon(u)
        )
    ...
    forcing = fe.Constant((0.0, 0.0, - DENSITY * ACCELERATION_DUE_TOGRAVITY))
    ...
    deviatoric_stess_tensor = (
        sigma(u_solution)
        -
        1/3 * fe.tr(sigma(u_solution)) * fe.Identity(3)
    )
    von_Mises_stress = fe.sqrt(3/2 * fe.inner(deviatoric_stess_tensor, deviatoric_stess_tensor))
    ...
    # Write out fields for visulatization with Paraview
